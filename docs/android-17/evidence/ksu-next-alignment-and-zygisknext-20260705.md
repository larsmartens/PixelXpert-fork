...
- `XPLauncher` hook registrations use `runSafe`; the Android 17 audio-focus workaround is also SDK-gated, so it does not bypass the boot defer path with an exception-propagating callback.
